...
import { FaHandHoldingUsd, FaSearch, FaMoneyBillWave, FaAlignLeft, FaCheck, FaTimes } from 'react-icons/fa';
...
const RequestMoney = () => {
  ...
  const { findUsers, requestMoney, loading } = useTransactions();
  ...
  const handleSubmit = async (e) => {
    ...
    // Request money
    const result = await requestMoney(selectedUser._id, parseFloat(amount), description);
    
    if (result.success) {
      setSuccess(`Successfully requested $${parseFloat(amount).toFixed(2)} from ${selectedUser.firstName} ${selectedUser.lastName}`);
      ...
      setError(result.message || 'Failed to request money');
    }
